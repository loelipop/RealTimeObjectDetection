...
public class objectDetectorClass {
    ...
    private ByteBuffer convertBitmapToByteBuffer(Bitmap bitmap) {
        ...
        boolean isQuantized = true; // Set this to false if the model expects floating-point input

        int inputSize = INPUT_SIZE;
        int numBytesPerChannel = isQuantized ? 1 : 4; // 1 byte per channel for quantized input, 4 bytes per channel for floating-point input

        byteBuffer = ByteBuffer.allocateDirect(inputSize * inputSize * PIXEL_SIZE * numBytesPerChannel);
        byteBuffer.order(ByteOrder.nativeOrder());

        // Log the size of the ByteBuffer
        Log.d("ByteBufferSize", "Size of ByteBuffer: " + byteBuffer.capacity() + " bytes");
        ...
        for (int i = 0; i < inputSize; ++i) {
            for (int j = 0; j < inputSize; ++j) {
                ...
                if (isQuantized) {
                    // Quantized input: normalize pixel values to [0, 255]
                    byteBuffer.put((byte) ((val >> 16) & 0xFF));
                    byteBuffer.put((byte) ((val >> 8) & 0xFF));
                    byteBuffer.put((byte) (val & 0xFF));
                } else {
                    // Floating-point input: normalize pixel values to [0, 1]
...
//This is objectDetectorClass.java